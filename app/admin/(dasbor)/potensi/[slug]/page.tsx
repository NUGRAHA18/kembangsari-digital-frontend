...
import {
  addPotentialImagesAction,
  setPrimaryImageAction,
  updatePotentialImageAction,
} from "@/app/admin/(dasbor)/potensi/actions";
...
import type { PotentialImage } from "@/types/api";
...
const MESSAGES: Record<string, string> = {
  dibuat: "Potensi berhasil disimpan. Sekarang unggah gambarnya.",
...
export default async function ManagePotentialPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold tracking-tight text-pretty">{potential.name}</h1>

        <div className="mt-2 flex flex-wrap items-center gap-3">
          <Badge tone={potential.isActive ? "primary" : "neutral"}>
            {potential.isActive ? "Tampil" : "Disembunyikan"}
          </Badge>
          <span className="text-sm text-muted">{humanizeEnum(potential.category)}</span>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          ...
          <Link
            href={`/admin/potensi/${potential.slug}/hapus`}
      ...
      <section aria-labelledby="gambar">
        ...
        <Card className="mb-4">
          <CardBody>
            <ImageUploadForm
              action={addPotentialImagesAction}
              hiddenFields={{ potentialId: potential.id, potentialSlug: potential.slug }}
              hint="Tampil di bagian dokumentasi halaman potensi. Gambar pertama otomatis menjadi gambar utama."
            />
          </CardBody>
        </Card>
        ...
        {images.length > 0 ? (
          ...
          <EmptyState
            title="Belum ada gambar dokumentasi"
            description="Halaman potensi ini tampil tanpa bagian dokumentasi sampai gambar pertama diunggah."
          />
        )}
      </section>
      ...
      <section aria-labelledby="data">
        ...
        <PotentialForm potential={potential} />
      </section>
    </div>
  );
}
...
function ImageCard({
  ...
  return (
    <Card className="h-full">
      ...
      <img
        src={image.url}
        alt={image.caption ?? "Gambar potensi"}
        loading="lazy"
      ...
      <CardBody className="flex flex-col gap-3 p-4">
        ...
        <form action={updatePotentialImageAction} className="flex flex-col gap-3">
          <input type="hidden" name="id" value={image.id} />
          <input type="hidden" name="potentialSlug" value={potentialSlug} />
        ...
        <Link
          href={`/admin/potensi/${potentialSlug}/gambar/${image.id}/hapus`}
